...
# MAGIC %md
# MAGIC ## DBX Model Registry Deployment

# COMMAND ----------

# MAGIC %md
# MAGIC ### Deploy New Model Version

# COMMAND ----------

import os
dbx_token = dbutils.secrets.get("ml_ops", "databricks_access_token")
dbx_manage_token = dbutils.secrets.get("ml_ops", "databricks_manage_token")
github_token = dbutils.secrets.get("ml_ops", "opticon_github_token")

os.environ['GITHUB_TOKEN'] = github_token
os.environ['DBX_TOKEN'] = dbx_manage_token 

# COMMAND ----------

# MAGIC %sh
# MAGIC export GITHUB_USER=dbubel
# MAGIC
# MAGIC pip install mlflow_deployment_client@git+https://${GITHUB_TOKEN}@github.com/RedVentures/mlflow-deployment-client.git@main
# MAGIC pip install git+https://${GITHUB_USER}:${GITHUB_TOKEN}@github.com/RedVentures/weaver-client-python@1.0.13

# COMMAND ----------

import weaver_client.client as weaver
import mlflow_deployment_client.client as MLFlowDeployClient
model_name = 'global-content-tags' if environment == "production" else 'global-content-tags-dev'
model_name
...
class Pyfunc_Model(mlflow.pyfunc.PythonModel):
  ...
  def predict(self, context, X):
    try:
      # Grab first row as dictionary
      row = X.to_dict(orient='records')[0]
      prediction, embedding = self.model.predict_proba(row, self.embed)
      return [{ 'predictions': prediction, 'embedding': embedding }]
    except:
...
data_input = {
  'url': "www.mattressguide.com",
  'text': "Mattresses are essential pieces of furniture designed to provide comfort and support for sleeping. They come in various types, including memory foam, innerspring, latex, and hybrid mattresses, each offering different levels of firmness and features. Memory foam mattresses contour to the body's shape, providing pressure relief and motion isolation, while innerspring mattresses offer bounce and support with coil springs. Latex mattresses are known for their durability and natural materials, offering a responsive and breathable sleeping surface. Hybrid mattresses combine different materials to provide a balance of comfort and support. Choosing the right mattress depends on individual preferences, such as sleeping position, body weight, and personal comfort needs. Regularly rotating and maintaining a mattress can help prolong its lifespan and ensure a restful night's sleep.",
  'title': "Exploring the World of Mattresses"
}
data_input_df = pd.DataFrame.from_records([data_input])
data_input_df

# COMMAND ----------

if manual_updates:
    test_model = Pyfunc_Model()
    test_model.setup_model(model)
    test_model.load_context(None) ##this automatically happens upon deployment, but cannot be deployed with this already loaded bc cloudpickle doesn't play nice with tensofrflow
    output = test_model.predict(None, data_input_df) #loading context to test and making sure a prediction works correctly
    assert output[0]['predictions'][2]['value'] == 'mattresses' ##are we returning decent predictions

    wrapped_model = Pyfunc_Model()
    wrapped_model.setup_model(model) ##wrapping the model to deploy without loading context
...
mlflow_conda = {
    ...
    'name': 'conda',
    'dependencies': [
      'python=3.9', # python version used by standard LTS 11.3
      'pip=23.0.1',
      {'pip':[
        'mlflow=={}'.format(mlflow.__version__), # include additional libraries in this list
        'cloudpickle=={}'.format(cloudpickle.__version__),
        'pandas=={}'.format(pd.__version__),
        'numpy=={}'.format(np.__version__),
        'tensorflow=={}'.format(tf.__version__),
        'tensorflow-estimator==2.11.0',
        'tensorflow_hub=={}'.format(hub.__version__),
        'tensorflow-text=={}'.format(tensorflow_text.__version__),
        'beautifulsoup4=={}'.format(bs4.__version__),
...
if manual_updates:
  with mlflow.start_run() as run:
    # Log the pyfunc model and register as new version
    mlflow.pyfunc.log_model(
        python_model=wrapped_model, # model created from above
        artifact_path=model_name, # path for context to find artifacts
        registered_model_name=model_name, # name of the model in DBX Model Registry
        conda_env=mlflow_conda, # python environment for the model to use
    )
  deploy_client = MLFlowDeployClient.MLFLowDeploymentClient(
      workspace_url='https://redventures-datascience-production-sk08h1.cloud.databricks.com', 
      endpoint_name= model_name,
      log_level='info'
  )
  try:
    response = deploy_client.increment_endpoint_model_version()
    print(response.text)
  except Exception as e:
    print("first model version: " + str(e))

  deploy_client.poll_for_endpoint_update()